...
//  Le web envoie: window.webkit.messageHandlers.pencilKit.postMessage({...})
//
...
class PencilKitMessageHandler: NSObject, WKScriptMessageHandler {
    ...
    private func handleUpdateTool(config: [String: Any]?) {
        guard let config = config else { return }
        ...
        coordinator?.updateTool(tool: tool, color: color, size: size, opacity: opacity)
    }
